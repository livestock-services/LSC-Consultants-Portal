    ...
    <div class="card  my-4">
      <header class="card-header footy my-4">
        <h1 class="card-header-title  header-text">
         Cattle Post Mortems between <span class="tag is-info is-light"> {{ startTime }} </span> and <span class="tag is-info is-light"> {{ endTime }} </span>
        </h1>
      </header>
     ...
     <b-tooltip label="Export to Excel" type="is-dark">
      ...
      <download-excel
       ...
       worksheet="Cattle Worksheet"
       type="xls"
       name = "Cattle Post Mortems.xls">
      ...
      <b-form v-model="cattleCard" class="card-content mx-4 my-4">
        <div class="content mx-6 has-text-left">
          
          <div class="columns">
            <div class=" my-4 pl-4">
           Anaplasmosis:
               <b-field  class=" tag is-primary mx-4"> {{ cattleAnaplasmosis }}</b-field>
...
<script>
import CattleFilterModal from '~/components/modals/Filter/cattle-filter-modal.vue'
...
//let filteredAgros = await this.getFilteredAgroRecords();
 ...
 //console.log(filteredAgros)
  ...
  methods:{
      ...mapActions('vetData', ['getAllPostMortemRecords','getFilteredCattlePMRecords', 'load']),
