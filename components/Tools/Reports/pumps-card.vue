<template>
      ...
      <div class="card  my-4">
        <header class="card-header footy my-4">
          <h1 class="card-header-title  header-text">
           Water Pump Consultations between <span class="tag is-info is-light"> {{ startTime }} </span> and <span class="tag is-info is-light"> {{ endTime }} </span>
          </h1>
        </header>
       ...
       <b-tooltip label="Export to Excel" type="is-dark">
        ...
        <b-form v-model="waterPumpCard" class="card-content mx-4 my-4">
          <div class="content mx-6 has-text-left">
            
            <div class="columns">
              <div class=" my-4 pl-4">
            Consultations:
                 <b-field  class=" tag is-primary mx-4"> {{ filteredWaterPumpConsults }}</b-field>
  ...
  <script>
  import WaterPumpFilterModal from '~/components/modals/Filter/water-pump-filter-modal.vue'
  ...
  export default {
    ...
    data(){
      ...
      var waterPumpConsults = computed(()=> this.filteredWaterPumpConsults)
      ...
      var totalConsults =  computed(
                            ()=> this.filteredWaterPumpConsults
    ...
    computed: {
        ...
        ...mapGetters('pumpData', {
         loading: 'loading',
         allWaterPumpConsults: 'allWaterPumpRecords',
  
         filteredWaterPumpConsults:'allFilteredWaterPumpRecords',
         
         
         startTime:'filteredWaterPumpStartTime',
         endTime:'filteredWaterPumpEndTime',
  ...
  // console.log(allAgros)
   //console.log(filteredAgros)
    ...
    methods:{
        ...mapActions('pumpData', ['getFilteredWaterPumpRecords', 'load']),
        ...
        setTimeout(() => {
          this.$buefy.modal.open({
            parent: this,
            component: WaterPumpFilterModal,
            hasModalCard: true,
            trapFocus: true,
            canCancel: ['x'],
            destroyOnHide: true,
